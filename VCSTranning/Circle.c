...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		{
			TextOut(hDC, 150, 50, L"This is a test", 14);
		...
		{
			hNewPen = CreatePen(PS_SOLID, 1, RGB(255, 0, 0));   // red, double width
			hOldPen = (HPEN)SelectObject(hDC, hNewPen);
			...
			for (int i = 1; i <= 3; ++i) {
				Ellipse(hDC, 350-i*10, 10, 550, 210);   // draw the circle
				Sleep(2000);
			}
		...
		if (Count >= 2)   // User has clicked at least twice.  Display a polygon.
		{
			...
			hNewPen = CreatePen(PS_SOLID, 2, RGB(0, 255, 0));   // red, double width
			hOldPen = (HPEN)SelectObject(hDC, hNewPen);
